...
export const useTabsState = defineStore(
  ...
  (): TabsState => {
    ...
    const delSingleKeyItem = (index: number, isBasedKey: boolean = false) => {
      ...
      /* 是否是基于key删除 */
      if (isBasedKey) {
        const findIndex = tabsKeyArr.value.findIndex((item) => item.meta.key == index)
    ...
    const delMultiKeyItem = (index: number[], tabId: number, isBasedKey: boolean = false) => {
      ...
      index.forEach((i, _index) => {
        /* 基于key为索引来匹配 */
        if (isBasedKey) {
          let findIndex = tabsKeyArr.value.findIndex((item) => item.meta.key == i)
      ...
      /* 激活标签 */
      activeKey.value = tabsKeyArr.value.findIndex((item) => item.meta.key == tabId)
    ...
    /* 根据路由获取当前页面的key */
    watchEffect(() => {
      activeKey.value = tabsKeyArr.value.findIndex((item) => item.name == router.currentRoute.value.name)
    })
