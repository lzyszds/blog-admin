...
interface customToolbar2 extends result {
  menus?: result[]
}
...
interface Editor {
  ...
  value: Ref<string>
  option: any,
  togglePreview: any, // 切换预览
  toggleFullscreen: any, // 切换全屏
  toggleHtml: any, // 切换html
  updateHistoryRange: any, // 更新历史记录范围
  updateCurrentHistoryRange: any, // 更新当前历史记录范围
}


export default (editor: Editor): customToolbar2[] => {
  ...
  return [
    ...
    { name: 'hr' },
    {
      ...
      action: () => {
        const exit = (event) => {
          if (event.key === 'Escape') {
            editor.toggleFullscreen().then(() => {
              document.removeEventListener('keydown', exit);
            });
          }
        };

        editor.togglePreview().then((value) => {
          if (value) {
            // 注册一次 `Escape` 退出事件
...

    {
      ...
      action: () => {
        ...
        editor.toggleFullscreen().then((value) => {
          if (value) {
            // 注册一次 `Escape` 退出事件
            document.addEventListener('keydown', exit);
          } else {
